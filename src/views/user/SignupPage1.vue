<template>
  <form class="signup" @submit.prevent="submit" method="POST">
    ...
    <div class="signup-box">
      <h3>이용 약관 <span class="terms">(필수)</span></h3>
      <div class="agreement-box">{{agreementBox1}}
      </div>
      <div class="agreement-check">
        <check-box id="agree1" value="이용약관에 동의"
          @checked="agree1" ref="agreementCheck1"/>
      </div>
    </div>
    <div class="signup-box">
      <h3>개인정보 수집 및 이용 <span class="terms">(필수)</span></h3>
      <div class="agreement-box">{{agreementBox2}}</div>
      <div class="agreement-check">
        <check-box id="agree2" value="개인정보 수집 및 이용에 동의"
          @checked="agree2" ref="agreementCheck2"/>
      </div>
    </div>
    <div class="signup-total">
      <check-box id="agree-total" value="모든 약관을 확인하고 전체 동의합니다."
        @checked="agreeTotal" ref="agreementTotal"/>
    </div>
    <div class="error" v-show="error">
      {{error}}
    </div>
    <footer>
      <default-button class="button" type="button" value="이전" @click.prevent="cancel"/>
...
export default{
  ...
  data() {
    return {
      agreementBox1: `제1조(목적)
      
표준약관 제10023호

이 약관은 {블루디블루} 회사(전자거래 사업자)가 운영하는 {블루디블루} 사이버 몰(이하 "몰"이라 한다)에서 제공하는 인터넷 관련 서비스(이하 "서비스"라 한다)를 이용함에 있어 사이버몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다.
※ 「PC통신등을 이용하는 전자거래에 대해서도 그 성질에 반하지 않는한 이 약관을 준용합니다」


제2조(정의)

① "몰"이란 {블루디블루} 회사가 재화 또는 용역을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화 또는 용역을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
1. 이 약관은 2020년 09월 04일부터 적용됩니다.`,
      agreementBox2: `회사는 회원가입, 민원 등 고객상담 처리, 본인확인(14세 미만 아동 확인) 등 의사소통을 위한 정보 활용 및 이벤트 등과 같은 마케팅용도 활용, 회원의 서비스 이용에 대한 통계, 이용자들의 개인정보를 통한 서비스 개발을 위해 아래와 같은 개인정보를 수집하고 있습니다.
  ...
  methods: {
    ...
    submit() {
      try {
        ...
        this.$router.push({name: 'user.signup.form', params: {agreements: true}})
      } catch(error) {
        this.error = error
      }
    },
    ...
    agree1(v) {
      this.error = ''
      this.agreementCheck1 = v
      if(this.agreementCheck1 && this.agreementCheck2) {
        this.$refs.agreementTotal.check()
      } else {
        this.$refs.agreementTotal.uncheck()
      }
    },
    agree2(v) {
      this.error = ''
      this.agreementCheck2 = v
      if(this.agreementCheck1 && this.agreementCheck2) {
        this.$refs.agreementTotal.check()
      } else {
        this.$refs.agreementTotal.uncheck()
    ...
    agreeTotal(v) {
      this.error = ''
      if(v === true) {
        this.agreementCheck1 = true
        this.$refs.agreementCheck1.check()
        this.agreementCheck2 = true
        this.$refs.agreementCheck2.check()
      } else {
        this.agreementCheck1 = false
        this.$refs.agreementCheck1.uncheck()
        this.agreementCheck2 = false
        this.$refs.agreementCheck2.uncheck()
      }
    },
    // Private Method
    _checkAgreement() {
      if(this.agreementCheck1 === false) {
        throw '(필수) "이용 약관"에 체크해주세요.'
      } else if(this.agreementCheck2 === false) {
        throw '(필수) "개인정보 수집 및 이용"을 체크해주세요.'
      }
    }
  }
}
